...
delta.QRD = calculateDiffusionCoefficient(N,R,theta,x,k);
% Diffusion coefficient with radiation correction
delta.QRDrc = calculateDiffusionCoefficient(N,R_radcorr,theta,x,k);
% Flat plane
Rflat = ones(size(R));
delta.f = calculateDiffusionCoefficient(N,Rflat,theta,x,k);
...
function delta = calculateDiffusionCoefficient(N,R,theta,x,k)
    augind = length(x)./N;
